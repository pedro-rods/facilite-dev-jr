...
/**
 * REST controller for managing {@link com.company.hrlite.domain.Address}.
 */
@RestController
...
public class AddressResource {
    ...
    public ResponseEntity<AddressDTO> createAddress(@Valid @RequestBody AddressDTO addressDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/addresses/" + addressDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, addressDTO.getId().toString()))
            .body(addressDTO);
    }
     ...
     */
    @GetMapping("")
    public List<AddressDTO> getAllAddresses(@RequestParam(name = "filter", required = false) String filter) {
        if ("employee-is-null".equals(filter)) {
            LOG.debug("REST request to get all Addresss where employee is null");
            return addressService.findAllWhereEmployeeIsNull();
        }
