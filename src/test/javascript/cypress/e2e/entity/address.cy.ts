...
describe('Address e2e test', () => {
  ...
  const addressSample = { cep: 'while yuc', street: 'The Poplars', city: 'Furmanside', uf: 'RR' };
  ...
  describe('new Address page', () => {
    ...
    it('should create an instance of Address', () => {
      cy.get(`[data-cy="cep"]`).type('loftily w');
      cy.get(`[data-cy="cep"]`).should('have.value', 'loftily w');

      cy.get(`[data-cy="street"]`).type('The Orchard');
      cy.get(`[data-cy="street"]`).should('have.value', 'The Orchard');

      cy.get(`[data-cy="number"]`).type('lightly in');
      cy.get(`[data-cy="number"]`).should('have.value', 'lightly in');

      cy.get(`[data-cy="complement"]`).type('supposing ameliorate save');
      cy.get(`[data-cy="complement"]`).should('have.value', 'supposing ameliorate save');

      cy.get(`[data-cy="district"]`).type('because loftily');
      cy.get(`[data-cy="district"]`).should('have.value', 'because loftily');

      cy.get(`[data-cy="city"]`).type('Gleasonshire');
      cy.get(`[data-cy="city"]`).should('have.value', 'Gleasonshire');

      cy.get(`[data-cy="uf"]`).select('CE');
